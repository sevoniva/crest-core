...
flowchart TB
  subgraph cluster["Kubernetes Namespace"]
    ...
    subgraph web["StatefulSet: crest"]
      web1["frontend pod"]
      web2["frontend pod"]
    end

    subgraph api["StatefulSet: crest-service"]
...
| 滚动发布中断 | 前端和后端都使用 StatefulSet `OrderedReady` 滚动更新，固定 `crest-0/1` 与 `crest-service-0/1` 两个 Pod 身份，避免发布窗口出现第 3 个同类 Pod；发布窗口内需由单 Pod 承载基础流量 |
