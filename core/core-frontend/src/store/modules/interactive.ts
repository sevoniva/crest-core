...
const apiMap = [queryTreeApi, queryTreeApi, datasetTree, listDatasources]
...
export const interactiveStore = defineStore('interactive', {
  ...
  getters: {
    ...
    dataset(): InnerInteractive {
      return this.data[2]
    },
    datasource(): InnerInteractive {
      return this.data[3]
    }
  },
  actions: {
    async setInteractive(param: BusiTreeRequest, resParam?: object) {
      ...
      if (!hasMenuAuth(flag) && !window.CrestBi && !appStore.getIsIframe) {
    ...
    async loadBusiInteractive() {
      ...
      for (let i = 0; i < busiFlagMap.length; i++) {
        ...
        if (window.CrestBi || appStore.getIsIframe || hasMenuAuth(i)) {
...
const convertInteractive = (list): InnerInteractive => {
  const treeNodes = Array.isArray(list) ? (list as unknown as BusiTreeNode[]) : []
  const result: InnerInteractive = {
    rootManage: (treeNodes[0]?.weight || 0) >= 7,
    anyManage: false,
    treeNodes,
    leafNodeCount: 0,
    menuAuth: true
