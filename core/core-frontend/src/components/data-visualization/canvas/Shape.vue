<template>
  ...
  >
    ...
    >
      ...
      <div
        class="shape-shadow"
        v-show="batchOptFlag && element.component !== 'Tabs'"
        @mousedown="batchSelected"
      ></div>
      <template v-if="boardMoveActive">
        <div
          v-show="!element.editing"
          class="crest-drag-area crest-drag-top"
          @mousedown="handleBoardMouseDownOnShape"
        />
        <div
          v-show="!element.editing && element.component !== 'Tabs'"
          class="crest-drag-area crest-drag-right"
          @mousedown="handleBoardMouseDownOnShape"
        />
        <div
          v-show="!element.editing && element.component !== 'Tabs'"
          class="crest-drag-area crest-drag-bottom"
          @mousedown="handleBoardMouseDownOnShape"
        />
        <div
          v-show="!element.editing && element.component !== 'Tabs'"
          class="crest-drag-area crest-drag-left"
          @mousedown="handleBoardMouseDownOnShape"
        />
      </template>
    </div>
...
/** 国际化翻译函数，提供组件操作提示文案 */
const { t } = useI18n()
/** 主画布仓库提供当前组件、编辑模式和画布交互状态 */
const dvMainStore = dvMainStoreWithOut()
/** 快照仓库用于记录拖拽、缩放和隐藏操作 */
const snapshotStore = snapshotStoreWithOut()
/** 右键菜单仓库用于在组件选中时关闭上下文菜单 */
...
/** 主画布状态引用，驱动选中、批量、联动、Tab 碰撞和全屏表现 */
const {
  curComponent,
  dvInfo,
  editMode,
...
/** 组件交互本地状态，记录 Tab 移入检测和碰撞阈值 */
const state = reactive({
  seriesIdMap: {
    id: ''
  },
  // 禁止移入Tab中的组件
  ignoreTabMoveComponent: ['crest-button', 'crest-reset-button', 'Tabs', 'GroupArea'],
  // 当画布在tab中是 宽度左右拓展的余量
...
const handleMouseDownOnShape = e => {
  ...
  const move = moveEvent => {
    ...
    rafId = requestAnimationFrame(() => {
      ...
      pos['top'] = top
      pos['left'] = left
      // 非主画布和非分组画布中，组件越过 Tab 横向边界时触发移出判断。
      // 仪表板下移可能只是调整布局空间，因此移出判断仅处理横向越界。
      // 大屏与仪表板使用不同坐标来源，避免缩放后拖拽位置偏移。
      // Tab 位于分组内时不允许组件移入或移出。
      if (
        !pTabGroupFlag &&
        pJoinTab &&
        !isMainCanvas(canvasId.value) &&
      ...
      if (['Tabs', 'Group'].includes(element.value.component)) {
...
const handleMouseDownOnPoint = (point, e) => {
  ...
  const move = moveEvent => {
    ...
    if (
      ...
      (['Tabs'].includes(element.value.component) && !element.value.resizeInnerKeep)
    ) {
      groupSizeStyleAdaptor(element.value)
    }
  ...
  const up = () => {
    ...
    if (['Tabs'].includes(element.value.component) && element.value.resizeInnerKeep) {
...
// resize 算法适配，根据拖拽点位置调整 curGap 对中心点、当前点和对称点的偏移
/** 根据矩阵间距和拖拽点方向计算缩放偏移补偿 */
  ...
  if (
    ...
    for (const item of nodes) {
      if (
        ...
        item.getAttribute('component-type') === 'Tabs'
      ) {
