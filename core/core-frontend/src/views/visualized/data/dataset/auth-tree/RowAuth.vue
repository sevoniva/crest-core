...
// 校验单个条件节点的必填项和值类型
const errorDetected = ({ enumValue, fieldType, filterType, term, value, name }) => {
  if (!name) {
...
const dfsSubmit = arr => {
  ...
  arr.forEach(ele => {
    ...
    if (child.length) {
      ...
      const { enumValue, fieldId, filterType, fieldType, term, timeType, value, name } = ele
      errorDetected({ fieldType, enumValue, filterType, term, value, name })
      if (fieldId) {
        items.push({
          enumValue: enumValue || [],
          fieldId,
