...
const { t } = useI18n()
const defaultAllowedDatasourceTypes = ['obOracle', 'Excel', 'ExcelRemote', 'API']
// 新建文件夹弹窗引用，保存 Excel 或新数据源时复用其目录选择流程
...
let isUpdate = false
// 标记表单是否发生过用户修改，关闭抽屉时据此确认是否离开
const changeUpdate = val => {
  isUpdate = val
}
const allowedDatasourceTypes = ref<string[]>(defaultAllowedDatasourceTypes)

// 抽屉当前步骤，0 为类型选择，1 为配置编辑
const activeStep = ref(0)
// 通用数据源详情表单引用
const detail = ref()
// 插件数据源桥接组件引用，通过 invokeMethod 调用插件表单方法
const pluginBridge = ref()
// 本地 Excel 数据源表单引用
const excel = ref()
// 远程 Excel 数据源表单引用
const excelRemote = ref()
// 最近使用的数据源类型，展示在类型选择首页
const latestUseTypes = ref<string[]>([])
// 当前左侧类型大类
...
const handleNodeClick = (data: Node) => {
  currentType.value = data.type as DsType
}

// 类型搜索关键字变化时过滤第二步的数据源类型树
...
const datasourceTree = computed(() => {
  return typeList
    .filter((ele, index) => databaseList.value[index]?.length)
    .map(ele => {
      return {
        name: nameMap[ele],
        type: ele
...
const datasourceTypes = () => {
  const arr = [[], [], [], [], []]
  dsTypes
    .filter(item => allowedDatasourceTypes.value.includes(item.type))
    .forEach(item => {
...
const loadAllowedDatasourceTypes = () => {
  listDatasourceType()
    .then(res => {
      ...
      const allowed = (list || [])
        ...
        .filter(Boolean)
      allowedDatasourceTypes.value = allowed.length ? allowed : defaultAllowedDatasourceTypes
      datasourceTypes()
    })
    .catch(() => {
      allowedDatasourceTypes.value = defaultAllowedDatasourceTypes
      datasourceTypes()
    })
}
loadAllowedDatasourceTypes()

// 当前插件数据源的静态资源索引
const pluginIndex = ref('')
// 已加载的数据源插件列表
const pluginDs = ref([])
// 接收插件数据源定义，并追加到对应数据源类型大类中
const loadDsPlugin = data => {
  pluginDs.value = data
  data.forEach(item => {
    const { name, category, type, icon, extraParams, staticMap } = item
    if (!allowedDatasourceTypes.value.includes(type)) {
      return
    }
    const node = {
      name,
...
// API 和远程 Excel 的内部步骤，区分基础配置与同步配置
const activeApiStep = ref(0)

// 进入下一步，并处理 API 与远程 Excel 的双层步骤规则
const setNextStep = () => {
...
// 完成 Excel 类数据源保存后回写父目录，并执行目录弹窗回调
...
// 从完成页继续创建数据源，保留上一次的父目录
const continueCreating = () => {
  showFinishPage.value = false
  init(null, pid.value)
}

// 根据系统配置决定保存后展示完成页还是直接关闭并刷新列表
...
// 上一步按钮入口，统一走确认和步骤回退逻辑
const prev = () => {
  prevConfirm()
}

// 处理步骤回退，API 和远程 Excel 的第二步会先回到配置步骤
const prevConfirm = () => {
  if (
    (currentDsType.value.includes('API') || currentDsType.value === 'ExcelRemote') &&
...
// 发起数据源连通性校验，按 API、插件、远程 Excel 和普通数据源分别组装请求
const validateDS = () => {
...
// 保存数据源主流程，按 Excel、远程 Excel、API、插件和普通数据库分支处理
const saveDS = () => {
  isUpdate = false
  const request = JSON.parse(JSON.stringify(form)) as any
  ...
  if (currentDsType.value === 'ExcelRemote') {
    ...
    for (let i = 0; i < request.apiConfiguration.length; i++) {
      if (
        request.apiConfiguration[i].displayTableName === '' ||
        request.apiConfiguration[i].displayTableName === undefined ||
        request.apiConfiguration[i].displayTableName === null
      ) {
        request.apiConfiguration[i].displayTableName =
          'api_' +
    ...
    <template #header="{ close }">
      ...
      <div v-if="!editDs" class="flex-center" style="width: 100%">
        <el-steps custom style="max-width: 500px; flex: 1" :active="activeStep" align-center>
          <el-step>
            <template #title>
              {{ t('datasetUi.select_data_source') }}
            </template>
          </el-step>
          <el-step>
            <template #title>
      ...
      <div class="ds-editor" :class="editDs && 'edit-ds'">
        ...
        <div class="editor-content" :class="(activeStep === 0 || editDs) && 'type-title'">
          <ds-type-list
            ...
            :allowed-datasource-types="allowedDatasourceTypes"
          ></ds-type-list>
          <editor-detail
            ref="detail"
            :form="form"
