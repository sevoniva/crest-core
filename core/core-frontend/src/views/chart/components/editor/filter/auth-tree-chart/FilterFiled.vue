...
const confirmTimeSelect = () => {
  ...
  if (relativeToCurrent !== 'custom') {
    ...
    item.value.timeValue = presetTimeLabel || relativeToCurrent || ''
...
<template>
  <div class="white-nowrap">
    <!-- 单条权限筛选条件按字段、筛选方式、操作符和值横向排列，便于权限规则快速扫描。 -->
    <div class="filed" @mouseover="showDel = true" @mouseleave="showDel = false">
      ...
      <div class="white-nowrap flex-align-center" style="position: relative" v-if="item.fieldId">
        <!-- 非时间字段支持逻辑筛选和枚举筛选，时间字段单独走固定/动态时间配置。 -->
        ...
        <template v-if="item.filterType === 'logic'">
          ...
          <template v-else-if="!['null', 'empty', 'not_null', 'not_empty'].includes(item.term)">
            <el-input
              v-if="
                item.fieldType === 1 && item.filterTypeTime === 'dynamicDate' && !item.timeValue
              "
              @click="handleClick"
              readonly
              class="w70 mar5"
            ...
            <el-tooltip
              class="item"
              v-else-if="item.fieldType === 1 && item.filterTypeTime === 'dynamicDate'"
              effect="light"
  ...
  <!-- 固定时间选择器独立挂载，避免和动态时间弹窗互相复用状态。 -->
  <TimeSetDialog @saveTime="saveTime" ref="timeDialogRef"></TimeSetDialog>
</template>

<style lang="less" scoped>
.white-nowrap {
  /* 筛选条件行不换行，保持权限规则在横向轴上连续展示。 */
  white-space: nowrap;
}
.filed {
  /* 条件项宽度随内容增长，便于多条条件并排时保留完整字段和值区域。 */
  width: fit-content;
  height: 41.4px;
...
.clearfix {
  /* 兼容浮动布局的清除方式，保证枚举面板底部按钮不被两栏内容覆盖。 */
  zoom: 1;
}
</style>
