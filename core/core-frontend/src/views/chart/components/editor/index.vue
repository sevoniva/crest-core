...
// 监听分页条数调整，保持表格配置和画布渲染同步
useEmitt({
  name: 'set-page-size',
  callback: args => onTablePageSizeChange(args)
})
// 编辑器入参承载当前图表、数据集树和主题模式
...
// 当前图表类型对应的下拉配置
const currentChartTypeOption = computed(() => state.chartTypeOptions[0])
// 图表类型变化后刷新类型配置，供轴面板和样式面板读取
watch(
  () => [view.value.type, view.value],
  newVal => {
    const viewConfig = getViewConfig(newVal[0])
    state.chartTypeOptions.splice(
      0,
      state.chartTypeOptions.length,
...
const addAxis = (e, axis: AxisType) => {
  ...
  if (view.value.type === 'bar-range' && (axis === 'yAxis' || axis === 'yAxisExt')) {
    ...
    if (list && list.length > 0) {
      let valid = true
      for (let i = 0; i < list.length; i++) {
        if (list[i].groupType === 'd' && list[i].fieldType === 1) {
          list[i].sort = 'asc'
        }
        if (
          !(list[i].groupType === 'q' || (list[i].groupType === 'd' && list[i].fieldType === 1))
        ) {
          list.splice(i, 1)
          i--
          valid = false
  ...
  if (typeValid && type === 'q' && view.value.type === 'indicator') {
    ...
    if (list && list.length > 0) {
      let valid = true
      for (let i = 0; i < list.length; i++) {
        if (list[i].fieldType !== 2 && list[i].fieldType !== 3) {
          list.splice(i, 1)
          i--
          valid = false
        }
        ...
        for (let i = 0; i < list.length; i++) {
          if (
            !(
              ...
              (tempType === 'q' || (tempType === 'd' && list[i].fieldType === tempFieldType))
            )
          ) {
            list.splice(i, 1)
            i--
...
const onTypeChange = (render, type) => {
  ...
  if (chartViewInstance) {
    Object.assign(
      view.value,
      chartViewInstance.setupDefaultOptions(view.value) as unknown as ChartObj
    )
    // 处理轴
...
const renderTableCellChange = debounce(() => {
  renderChart(view.value)
}, 250)
// 表格单元格样式变化后处理合并字段的防抖重绘
const onTableCellChange = (val, prop) => {
  view.value.customAttr.tableCell = val
  if (prop === 'mergeFields') {
    renderTableCellChange()
    return
  }
  renderTableCellChange.cancel()
  renderChart(view.value)
}
// 汇总行样式变化后更新表格配置并重绘
...
const saveResultFilter = () => {
  if (
    ((state.filterItem.fieldType === 0 || state.filterItem.fieldType === 5) &&
      state.filterItem.filterType !== 'enum') ||
    state.filterItem.fieldType === 1 ||
    state.filterItem.fieldType === 2 ||
    state.filterItem.fieldType === 3
  ) {
    for (let i = 0; i < state.filterItem.filter.length; i++) {
      ...
      if (state.filterItem.fieldType === 2 || state.filterItem.fieldType === 3) {
        if (!f.term.includes('null') && !f.term.includes('empty') && isNaN(f.value)) {
          ElMessage.error(t('chart.filter_value_can_not_str'))
          return
        }
...
// 根据字段类型和计算字段标识选择图标
const getIconName = (fieldTypeCode, extField, dimension = false) => {
  if (extField === 2) {
...
<template>
  <div class="chart-edit" :class="'editor-' + themes" @keydown.stop @keyup.stop>
    ...
    <!--pc端设计使用-->
    <template v-if="!mobileInPc">
      <el-row v-loading="loading" class="crest-chart-editor">
        <div
          class="content-area"
          :class="{
                            ...
                            >
                              <template #prefix>
                                <Icon
                                  ...
                                  v-if="currentChartTypeOption?.isPlugin"
                                  :static-content="currentChartTypeOption?.icon"
                                />
                                <Icon
                                  v-else-if="currentChartTypeOption?.icon"
                                  class-name="chart-type-select-icon"
                                >
                                  <component
                                    class="svg-icon chart-type-select-icon"
                                    :is="iconChartMap[currentChartTypeOption.icon]"
                                  ></component>
                                </Icon>
                              </template>
                              <template #default>
          ...
          >
            ...
            <el-main class="dataset-main-top">
              <el-row class="dataset-select">
                ...
                <el-tooltip
                  :effect="toolTip"
                  :content="$t('datasetUi.edit_dataset')"
                  placement="top"
                >
                  <el-icon
                    v-if="curDatasetWeight >= 7 && !isCrestBi"
                        ...
                        <el-icon>
                          <Icon
                            ><component
                              class="svg-icon"
                              :class="`field-icon-${
                                fieldType[[2, 3].includes(element.fieldType) ? 2 : 0]
                              }`"
                              :is="getIconName(element.fieldType, element.extField)"
                            ></component
                          ></Icon>
                        </el-icon>
                        <span
                            ...
                            <el-icon>
                              <Icon
                                ><component
                                  class="svg-icon"
                                  :class="`field-icon-${
                                    fieldType[[2, 3].includes(ele.fieldType) ? 2 : 0]
                                  }`"
                ...
                >
                  ...
                  <el-scrollbar class="drag-list">
                    ...
                    >
                      ...
                      >
                        <el-icon>
                          <Icon :class-name="`field-icon-${fieldType[element.fieldType]}`"
                            ><component
                              class="svg-icon"
                              :class="`field-icon-${fieldType[element.fieldType]}`"
                              :is="getIconName(element.fieldType, element.extField, true)"
                            ></component
                          ></Icon>
                        </el-icon>
                        <span
                      ...
                      >
                        <template v-if="isDrag">
                          ...
                          >
                            <el-icon>
                              <Icon :class-name="`field-icon-${fieldType[ele.fieldType]}`"
                                ><component
                                  class="svg-icon"
                                  :class-name="`field-icon-${fieldType[ele.fieldType]}`"
                                  :is="iconFieldMap[fieldType[ele.fieldType]]"
                                ></component
                              ></Icon>
                            </el-icon>
