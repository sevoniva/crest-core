 ...
 * 可参与数值校验的字段类型
 */
const NUMERIC_FIELD_TYPES = [2, 3]

/**
 * 获取当前 Y 轴字段或第一个可选指标字段
 */
...
const isNumType = field => {
  return quotaData.value.find(
    ele => ele.id === field.id && NUMERIC_FIELD_TYPES.includes(ele.fieldType)
  )
}
