...
import SelectScreenDialog from '@/custom-component/screen/SelectScreenDialog.vue'
/** 可视化主画布状态仓库 */
...
const initCarousel = () => {
  ...
  if (!isEditMode.value) {
    if (element.value.carousel?.enable) {
      ...
      const tabs = element.value.propValue || []
      ...
      carouselTimer = setInterval(() => {
        // 鼠标移入时停止轮播
        if (!state.hoverFlag) {
          const nowIndex = switchCount % tabs.length
          switchCount++
          nextTick(() => {
            element.value.editableTabsValue = tabs[nowIndex].name
          })
        }
      }, switchTime)
    }
