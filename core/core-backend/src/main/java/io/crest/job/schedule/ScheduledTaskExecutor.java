...
public class ScheduledTaskExecutor {
    ...
    private ObjectProvider<ScheduledReportTaskRunner> reportTaskRunnerProvider;
    ...
    public boolean execute(Map<String, Object> taskData, String queueJobKey, Long scheduledFireTime) {
        if (queueService != null && queueService.enabled()) {
            queueService.enqueueReportTask(queueTaskData(taskData, queueJobKey, scheduledFireTime));
            return true;
        }
        return runReportTask(taskData, true);
    }

    // Worker 队列路径必须真实执行；未接入执行器时交由队列侧进入死信。
    public boolean executeQueued(Map<String, Object> taskData) {
        return runReportTask(taskData, false);
    }

    private boolean runReportTask(Map<String, Object> taskData, boolean keepScheduleWhenMissingRunner) {
        ScheduledReportTaskRunner runner = reportTaskRunner();
        if (runner != null) {
            return runner.executeReportTask(taskData);
        }
        LogUtil.warn("Report task runner is not configured");
        return keepScheduleWhenMissingRunner;
    }

    private ScheduledReportTaskRunner reportTaskRunner() {
        return reportTaskRunnerProvider == null ? null : reportTaskRunnerProvider.getIfAvailable();
    }
    ...
    public void addThresholdTask(Long taskId, String cron) {
        ...
        scheduleManager.addOrUpdateCronJob(jobKey, triggerKey, ScheduledTaskJob.class, cron, startDate, null, jobDataMap);
    }
    ...
    public void addOrUpdateTask(Long taskId, String cron, Long startTime, Long endTime) {
        ...
        scheduleManager.addOrUpdateCronJob(jobKey, triggerKey, ScheduledTaskJob.class, cron, new Date(startTime), end, jobDataMap);
    }
    ...
    public void addRetryTask(Long taskId, Integer retryLimit, Integer retryInterval, Object retryParam) {
        ...
        Date end = null;
        if (ObjectUtils.isNotEmpty(endTime)) end = new Date(endTime);
        scheduleManager.addOrUpdateCronJob(jobKey, triggerKey, ScheduledTaskJob.class, cron, new Date(now), end, jobDataMap);
    }
    ...
    public void addTempTask(Long taskId, Long startTime) {
        ...
        jobDataMap.put("taskId", taskId);
        scheduleManager.addOrUpdateCronJob(jobKey, triggerKey, ScheduledTaskJob.class, cron, new Date(startTime), null, jobDataMap);
    }
