...
CREATE TABLE dataset_group (
  ...
  create_by VARCHAR2(50) DEFAULT NULL,
  create_time NUMBER(19,0) DEFAULT NULL,
  qrtz_instance VARCHAR2(1024) DEFAULT NULL,
...
CREATE TABLE dataset_sync_task (
...
CREATE TABLE dataset_sync_task_log (
  id NUMBER(19,0) GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  dataset_group_id NUMBER(19,0) NOT NULL,
...
CREATE TABLE data_asset_profile (
  id NUMBER(19,0) NOT NULL,
...
CREATE TABLE data_source_sync_task_log (
  id NUMBER(19,0) GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  ds_id NUMBER(19,0) NOT NULL,
  task_id NUMBER(19,0) DEFAULT NULL,
  start_time NUMBER(19,0) DEFAULT NULL,
...
CREATE TABLE data_source_driver (
  id NUMBER(19,0) GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  name VARCHAR2(50) NOT NULL,
  create_time NUMBER(19,0) NOT NULL,
  type VARCHAR2(255) DEFAULT NULL,
...
CREATE TABLE data_source_driver_jar (
  id NUMBER(19,0) GENERATED BY DEFAULT AS IDENTITY NOT NULL,
...
CREATE TABLE data_source_finish_page (
  id NUMBER(19,0) GENERATED BY DEFAULT AS IDENTITY NOT NULL,
  PRIMARY KEY (id)
);

CREATE TABLE export_download_task (
  id VARCHAR2(255) NOT NULL,
  create_time NUMBER(19,0) DEFAULT NULL,
...
CREATE TABLE workspace_recent_resource (
  id NUMBER(19,0) NOT NULL,
...
CREATE TABLE workspace_favorite_resource (
  id NUMBER(19,0) NOT NULL,
  resource_id NUMBER(19,0) NOT NULL,
  "UID" NUMBER(19,0) NOT NULL,
  resource_type NUMBER(10,0) NOT NULL,
...
CREATE TABLE threshold_instance (
  id NUMBER(19,0) NOT NULL,
...
CREATE TABLE bi_chart_snapshot (
  id NUMBER(19,0) NOT NULL,
  title VARCHAR2(1024) DEFAULT NULL,
  scene_id NUMBER(19,0) NOT NULL,
  table_id NUMBER(19,0) DEFAULT NULL,
...
CREATE TABLE bi_visualization_snapshot (
  id VARCHAR2(50) NOT NULL,
  name VARCHAR2(255) DEFAULT NULL,
  pid VARCHAR2(50) DEFAULT NULL,
  org_id VARCHAR2(50) DEFAULT NULL,
...
CREATE TABLE auth_sso_identity_binding (
  id NUMBER(19,0) NOT NULL,
  user_id NUMBER(19,0) NOT NULL,
...
CREATE TABLE iam_role_menu_permission (
  id NUMBER(19,0) NOT NULL,
  rid NUMBER(19,0) NOT NULL,
...
CREATE TABLE iam_resource_index (
  id NUMBER(19,0) NOT NULL,
...
CREATE INDEX idx_dataset_sync_task_log_A ON dataset_sync_task_log (dataset_group_id,task_id,start_time);
CREATE INDEX idx_data_asset_profile_status ON data_asset_profile (asset_type, certified, recommended, deprecated);
CREATE INDEX idx_data_asset_profile_owner ON data_asset_profile (owner_id);
CREATE INDEX idx_dataset_table_task_log_ds_id ON data_source_sync_task_log (ds_id);
CREATE INDEX idx_dataset_table_task_log_task_id ON data_source_sync_task_log (task_id);
CREATE INDEX idx_dataset_table_task_log_A ON data_source_sync_task_log (ds_id,table_name,start_time);
CREATE INDEX idx_sso_provider_type ON auth_sso_provider (provider_type, enabled);
CREATE INDEX idx_sso_identity_user ON auth_sso_identity_binding (user_id, provider_id);
CREATE INDEX idx_sso_identity_account ON auth_sso_identity_binding (account);
CREATE INDEX idx_operation_time ON audit_log (operation_time);
CREATE INDEX idx_operator_id ON audit_log (operator_id);
CREATE INDEX idx_operation_type ON audit_log (operation_type);
CREATE INDEX idx_resource_type ON audit_log (resource_type);
CREATE INDEX idx_crest_org_pid ON iam_org (pid);
CREATE INDEX idx_crest_org_path ON iam_org (path);
CREATE INDEX idx_crest_role_oid ON iam_role (oid);
CREATE INDEX idx_crest_user_org_oid ON iam_user_org (oid);
CREATE INDEX idx_crest_user_role_rid ON iam_user_role (rid);
CREATE INDEX idx_crest_role_menu_id ON iam_role_menu_permission (menu_id);
CREATE INDEX idx_crest_resource_oid_type ON iam_resource_index (oid, resource_type);
CREATE INDEX idx_crest_resource_creator ON iam_resource_index (creator);
CREATE INDEX idx_crest_resource_permission_target ON iam_resource_permission (target_type, target_id);
...
-- Baseline seed data
-- Seed for ref_area is loaded by ObOracleMetadataSeedInitializer.

UPDATE data_asset_profile SET certified = 0, recommended = 0 WHERE deprecated = 1;

INSERT INTO data_engine VALUES (1,'默认引擎','默认引擎','obOracle','',NULL,NULL,NULL,NULL,1);

INSERT INTO font_asset VALUES (1,'PingFang',NULL,NULL,1,0,1,NULL,NULL);

INSERT INTO iam_menu VALUES (1,0,2,'workbranch','workbranch',1,NULL,'/workbranch',0,1,1);
INSERT INTO iam_menu VALUES (2,0,2,'panel','visualized/view/panel',2,NULL,'/panel',0,1,1);
INSERT INTO iam_menu VALUES (3,0,2,'screen','visualized/view/screen',3,NULL,'/screen',0,1,1);
INSERT INTO iam_menu VALUES (4,0,1,'data',NULL,5,NULL,'/data',0,1,0);
INSERT INTO iam_menu VALUES (5,4,2,'dataset','visualized/data/dataset',1,NULL,'/dataset',0,1,1);
INSERT INTO iam_menu VALUES (6,4,2,'datasource','visualized/data/datasource',2,NULL,'/datasource',0,1,1);
INSERT INTO iam_menu VALUES (11,0,2,'dataset-form','visualized/data/dataset/form',7,NULL,'/dataset-form',1,0,0);
INSERT INTO iam_menu VALUES (12,0,2,'datasource-form','visualized/data/datasource/form',7,NULL,'/ds-form',1,0,0);
INSERT INTO iam_menu VALUES (15,0,1,'sys-setting',NULL,6,NULL,'/sys-setting',1,1,0);
INSERT INTO iam_menu VALUES (16,15,2,'parameter','system/parameter',1,'sys-parameter','/parameter',0,1,0);
INSERT INTO iam_menu VALUES (66,0,2,'association','visualized/data/lineage',4,'association','/lineage',0,1,1);
INSERT INTO iam_menu VALUES (67,15,2,'share-management','system/share',2,'icon_share-label_outlined','/share-management',0,1,1);
INSERT INTO iam_menu VALUES (68,15,2,'site-setting','system/site',3,'tab-title','/site-setting',0,1,1);
INSERT INTO iam_menu VALUES (69,15,2,'user-management','system/user',4,'icon_member_filled','/user-management',0,1,1);
INSERT INTO iam_menu VALUES (70,0,1,'msg',NULL,200,NULL,'/msg',1,1,0);


INSERT INTO sys_setting VALUES (1,'basic.dsIntervalTime','6','text',11);
INSERT INTO sys_setting VALUES (2,'basic.dsExecuteTime','minute','text',3);
INSERT INTO sys_setting VALUES (3,'ai.baseUrl','','text',0);
INSERT INTO sys_setting VALUES (7,'template.url','','text',0);
INSERT INTO sys_setting VALUES (8,'template.accessKey','crest','text',1);
INSERT INTO sys_setting VALUES (9,'basic.frontTimeOut','60','text',1);
INSERT INTO sys_setting VALUES (10,'basic.exportFileLiveTime','30','text',2);
INSERT INTO sys_setting VALUES (100102206800000001,'basic.siteTitle','Crest','text',9);
INSERT INTO sys_setting VALUES (1048232869488627717,'basic.shareDisable','false','text',11);
INSERT INTO sys_setting VALUES (1048232869488627718,'basic.sharePeRequire','false','text',12);
INSERT INTO sys_setting VALUES (1048232869488627719,'basic.defaultSort','1','text',13);
INSERT INTO sys_setting VALUES (1048232869488627720,'basic.defaultOpen','0','text',14);
INSERT INTO sys_setting VALUES (1048232869488627721,'basic.initialPassword','','pwd',15);
INSERT INTO sys_setting VALUES (1048232869488627722,'basic.pwdStrategy','true','text',16);
INSERT INTO sys_setting VALUES (1048232869488627723,'basic.dip','false','text',17);
INSERT INTO sys_setting VALUES (1048232869488627724,'basic.pvp','0','text',18);
INSERT INTO sys_setting VALUES (1048232869488627725,'basic.loginLimit','false','text',19);
INSERT INTO sys_setting VALUES (1048232869488627726,'basic.loginLimitRate','5','text',20);
INSERT INTO sys_setting VALUES (1048232869488627727,'basic.loginLimitTime','30','text',21);

INSERT INTO startup_job VALUES ('chartFilterDynamic','chartFilterDynamic','done');
INSERT INTO startup_job VALUES ('chartFilterMerge','chartFilterMerge','done');
INSERT INTO startup_job VALUES ('datasetCrossListener','datasetCrossListener','done');

INSERT INTO iam_user (id, account, name, email, phone_prefix, phone, password_hash, enable, is_admin, origin, create_time, update_time)
...
-- Seed for visualization_subject is loaded by ObOracleMetadataSeedInitializer.
...
INSERT INTO iam_menu VALUES (71,15,2,'single-sign-on','system/sso',5,'authentication','/single-sign-on',0,1,1);

INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000001,'sso.enabled','false','text',1);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000002,'sso.providerName','企业单点登录','text',2);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000003,'sso.clientId','','text',3);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000004,'sso.clientSecret','','text',4);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000005,'sso.authorizationEndpoint','','text',5);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000006,'sso.tokenEndpoint','','text',6);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000007,'sso.userInfoEndpoint','','text',7);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000008,'sso.issuer','','text',8);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000009,'sso.scope','openid profile email','text',9);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000010,'sso.redirectUri','','text',10);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000011,'sso.userIdAttribute','sub','text',11);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000012,'sso.accountAttribute','preferred_username','text',12);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000013,'sso.nameAttribute','name','text',13);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000014,'sso.emailAttribute','email','text',14);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000015,'sso.autoCreateUser','true','text',15);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000016,'sso.allowLocalLogin','true','text',16);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000017,'sso.requireHttps','true','text',17);
INSERT INTO sys_setting (id, pkey, pval, type, sort) VALUES (100140000000000018,'sso.logoutRedirectUrl','','text',18);
...
INSERT INTO auth_sso_provider (id, provider_key, provider_type, name, enabled, create_time, update_time) VALUES (1, 'default', 'OIDC_GENERIC', '统一身份认证', 1, 0, 0);

INSERT INTO sys_setting (id, pkey, pval, type, sort)
SELECT
    100140000000000019,
    'sso.providerType',
    CASE
...
WHERE NOT EXISTS (
    SELECT 1 FROM (
        SELECT id FROM sys_setting WHERE pkey = 'sso.providerType'
    ) existing_setting
);
...
WHERE NOT EXISTS (
    SELECT 1 FROM (
        SELECT id FROM sys_setting WHERE pkey = 'sso.unionIdAttribute'
    ) existing_setting
);

UPDATE auth_sso_provider
SET
    provider_type = COALESCE((SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.providerType' FETCH FIRST 1 ROWS ONLY), provider_type),
    name = COALESCE((SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.providerName' FETCH FIRST 1 ROWS ONLY), name),
    enabled = COALESCE((SELECT CASE WHEN LOWER(DBMS_LOB.SUBSTR(pval, 4000, 1)) = 'true' THEN 1 ELSE 0 END FROM sys_setting WHERE pkey = 'sso.enabled' FETCH FIRST 1 ROWS ONLY), enabled),
    client_id = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.clientId' FETCH FIRST 1 ROWS ONLY),
    client_secret = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.clientSecret' FETCH FIRST 1 ROWS ONLY),
    authorization_endpoint = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.authorizationEndpoint' FETCH FIRST 1 ROWS ONLY),
    token_endpoint = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.tokenEndpoint' FETCH FIRST 1 ROWS ONLY),
    user_info_endpoint = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.userInfoEndpoint' FETCH FIRST 1 ROWS ONLY),
    issuer = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.issuer' FETCH FIRST 1 ROWS ONLY),
    scope = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.scope' FETCH FIRST 1 ROWS ONLY),
    redirect_uri = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.redirectUri' FETCH FIRST 1 ROWS ONLY),
    user_id_attribute = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.userIdAttribute' FETCH FIRST 1 ROWS ONLY),
    account_attribute = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.accountAttribute' FETCH FIRST 1 ROWS ONLY),
    name_attribute = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.nameAttribute' FETCH FIRST 1 ROWS ONLY),
    email_attribute = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.emailAttribute' FETCH FIRST 1 ROWS ONLY),
    union_id_attribute = (SELECT DBMS_LOB.SUBSTR(pval, 4000, 1) FROM sys_setting WHERE pkey = 'sso.unionIdAttribute' FETCH FIRST 1 ROWS ONLY),
    auto_create_user = COALESCE((SELECT CASE WHEN LOWER(DBMS_LOB.SUBSTR(pval, 4000, 1)) = 'true' THEN 1 ELSE 0 END FROM sys_setting WHERE pkey = 'sso.autoCreateUser' FETCH FIRST 1 ROWS ONLY), auto_create_user),
    require_https = COALESCE((SELECT CASE WHEN LOWER(DBMS_LOB.SUBSTR(pval, 4000, 1)) = 'true' THEN 1 ELSE 0 END FROM sys_setting WHERE pkey = 'sso.requireHttps' FETCH FIRST 1 ROWS ONLY), require_https),
    update_time = ROUND((CAST(SYSTIMESTAMP AS DATE) - DATE '1970-01-01') * 86400000)
WHERE provider_key = 'default';

INSERT INTO iam_org (id, pid, name, code, path, "LEVEL", sort, enable, readonly, create_time, update_time) VALUES (1, 0, '默认组织', 'default', '/1/', 0, 0, 1, 1, 1779664240000, 1779664240000);

INSERT INTO iam_role (id, oid, name, code, description, type_code, readonly, system_role, org_admin, create_time, update_time) VALUES (1, 1, '系统管理员', 'system_admin', '拥有全部系统管理和资源管理权限', 1, 1, 1, 1, 1779664240000, 1779664240000);
INSERT INTO iam_role (id, oid, name, code, description, type_code, readonly, system_role, org_admin, create_time, update_time) VALUES (2, 1, '普通用户', 'member', '默认业务使用角色', 2, 0, 1, 0, 1779664240000, 1779664240000);
INSERT INTO iam_role (id, oid, name, code, description, type_code, readonly, system_role, org_admin, create_time, update_time) VALUES (3, 1, '审计只读', 'auditor', '面向审计和巡检场景的只读角色', 3, 1, 1, 0, 1779664240000, 1779664240000);

INSERT INTO iam_menu (id, pid, type, name, component, menu_sort, icon, path, hidden, in_layout, auth) VALUES (73, 15, 2, 'org-management', 'system/org', 5, 'org', '/org-management', 0, 1, 1);
INSERT INTO iam_menu (id, pid, type, name, component, menu_sort, icon, path, hidden, in_layout, auth) VALUES (74, 15, 2, 'role-management', 'system/role', 6, 'peoples', '/role-management', 0, 1, 1);
INSERT INTO iam_menu (id, pid, type, name, component, menu_sort, icon, path, hidden, in_layout, auth) VALUES (75, 15, 2, 'permission-management', 'system/permission', 7, 'auth', '/permission-management', 0, 1, 1);
INSERT INTO iam_menu (id, pid, type, name, component, menu_sort, icon, path, hidden, in_layout, auth) VALUES (76, 15, 2, 'audit-log', 'system/audit-log', 8, 'log', '/audit-log', 0, 1, 1);
INSERT INTO iam_menu (id, pid, type, name, component, menu_sort, icon, path, hidden, in_layout, auth) VALUES (77, 4, 2, 'data-asset', 'visualized/data/asset', 3, 'association', '/asset', 0, 1, 1);
INSERT INTO iam_menu (id, pid, type, name, component, menu_sort, icon, path, hidden, in_layout, auth) VALUES (78, 4, 2, 'cache-task', 'visualized/data/cache-task', 4, 'task', '/cache-task', 0, 1, 1);

INSERT INTO iam_user_org (id, "UID", oid, default_org, create_time)
SELECT id, id, 1, 1, COALESCE(create_time, 1779664240000) FROM iam_user;

INSERT INTO iam_user_role (id, "UID", oid, rid, create_time)
...
INSERT INTO iam_role_menu_permission (id, rid, menu_id, permission, create_time)
SELECT id * 100 + 1, 1, id, 'manage', 1779664240000 FROM iam_menu WHERE auth = 1 OR id IN (15, 16, 64, 67, 68, 69, 71, 72);

INSERT INTO iam_role_menu_permission (id, rid, menu_id, permission, create_time)
SELECT id * 100 + 2, 2, id, 'read', 1779664240000 FROM iam_menu WHERE auth = 1 AND id NOT IN (15, 16, 64, 67, 68, 69, 71, 72, 73, 74, 75, 76);

INSERT INTO iam_role_menu_permission (id, rid, menu_id, permission, create_time)
SELECT id * 100 + 3, 3, id, 'read', 1779664240000 FROM iam_menu WHERE auth = 1 AND id NOT IN (15, 16, 64, 67, 68, 69, 71, 72, 73, 74, 75);

INSERT INTO iam_resource_index (id, resource_id, resource_type, oid, creator, name, create_time, update_time)
...
DELETE FROM iam_role_menu_permission WHERE menu_id = 73;

DELETE FROM iam_menu WHERE id = 73;
