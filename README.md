...
Crest Core 是面向私有化生产环境的轻量 BI 平台。首版按全新环境交付设计，默认使用 OpenJDK 17、OceanBase Oracle 系统库、OceanBase Oracle 业务数据源、外部 Redis Cluster 和 Kubernetes 多副本部署。外部项目名使用 `Crest Core`，内部包名、表名前缀、配置前缀、镜像名和 Kubernetes 资源名继续保持 `crest`，避免在代码和运行时引入无价值的改名风险。
...
- 生产交付以 Kubernetes 为唯一主路径，旧安装器、离线包和 compose 交付不作为首版目标。
