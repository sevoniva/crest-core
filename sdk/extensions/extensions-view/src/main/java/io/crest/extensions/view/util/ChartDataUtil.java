    ...
    public static List<String[]> resultCustomSort(List<ChartViewFieldDTO> xAxis,List<ChartViewFieldDTO> yAxis, List<SortAxis> sortPriority, List<String[]> data) {
        ...
        if (axisList.size() > 0) {
            ...
            for (int i = 0; i < axisList.size(); i++) {
                ...
                if (Strings.CI.equals(item.getSort(), "custom_sort") && Objects.equals(item.getGroupType(), "d")) {
                    // 获取自定义值与data对应列的结果
                    if (i > 0) {
                        ...
                        for (String[] d : res) {
                            ...
                            for (int j = 0; j < i; j++) {
                                if (Strings.CI.equals(axisList.get(j).getSort(), "none")) {
                                    continue;
                                }
    ...
    public static Map<String, Object> transTableNormal(List<ChartViewFieldDTO> fields, ChartViewDTO view, List<String[]> data, Map<String, ColumnPermissionItem> desensitizationList) {
        ...
        data.forEach(ele -> {
            ...
            for (int i = 0; i < fields.size(); i++) {
                if (ObjectUtils.isNotEmpty(desensitizationList.keySet()) && desensitizationList.keySet().contains(fields.get(i).getEngineFieldName())) {
                    d.put(fields.get(i).getEngineFieldName(), desensitizationValue(desensitizationList.get(fields.get(i).getEngineFieldName()), String.valueOf(ele[i])));
                    continue;
                }
                if (i == ele.length) break;
                ChartViewFieldDTO chartViewFieldDTO = fields.get(i);
                if (chartViewFieldDTO.getFieldType() == 0 || chartViewFieldDTO.getFieldType() == 1 || chartViewFieldDTO.getFieldType() == 5) {
